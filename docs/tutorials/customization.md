...
NotQuests stores item choices in a portable format shared by Paper and NeoForge. Material selections work on both platforms. Exact item data is preserved using the platform's native item format where supported, including Paper custom-plugin items and NeoForge item components.
...
Every GUI lives as its own editable `.yml` file in the `guis/` folder inside the NotQuests data folder. In there you can rearrange the whole layout, swap materials and move buttons around — the `structure:` lines are a little visual map of the slots, and the `icons:` / `items:` sections define what goes in them.
...
Colors are supported basically everywhere in NotQuests through [MiniMessage](https://docs.advntr.dev/minimessage/format.html). Use tags such as `<gold>`, `<#ff9900>`, gradients, and the configurable NotQuests color tags instead of legacy `&6`/`&c` codes.

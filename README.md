...
Tool versions are pinned for local development through `mise.toml`. For asdf-compatible tools, `.tool-versions` pins Node.js. Cloudflare Pages does not install pnpm from `.tool-versions`; set `PNPM_VERSION=11.24.0` in the Pages build environment so Cloudflare uses the same pnpm version as local builds.
